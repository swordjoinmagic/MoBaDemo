...
- [2. 输入系统架构与描述](#2-输入系统架构与描述)
- [2.1. 在外部如何组织数据](#21-在外部如何组织数据)
- [2.2. 如何将外部数据输入进Unity](#22-如何将外部数据输入进unity)
    - [2.2.1. 如何初始化](#221-如何初始化)
- [2.3. JSON数据组织](#23-json数据组织)
    - [2.3.1. 输出系统架构与描述](#231-输出系统架构与描述)
    - [2.3.2. 技能](#232-技能)
    - [2.3.3. 所有JSON数据的组织形式](#233-所有json数据的组织形式)
- [3. GamePlay数据类](#3-gameplay数据类)
- [3.1. GamePlay数据类中的Model类和Mono类](#31-gameplay数据类中的model类和mono类)
    - [3.1.1. 为什么要引入看起来变扭的Model类和Mono类](#311-为什么要引入看起来变扭的model类和mono类)
    - [3.1.2. Model类和Mono类应该遵循怎样的设计原则](#312-model类和mono类应该遵循怎样的设计原则)
- [3.2. 技能设置描述](#32-技能设置描述)
- [3.3. 技能](#33-技能)
    - [3.3.1. ***基类 BaseSkill***](#331-基类-baseskill)
    - [3.3.2. ***主动技能类 ActiveSkill < BaseSkill***](#332-主动技能类-activeskill--baseskill)
    - [3.3.3. 技能分类](#333-技能分类)
- [3.4. 技能系统架构描述](#34-技能系统架构描述)
    - [3.4.1. 概述](#341-概述)
        - [3.4.1.1. 主动技能](#3411-主动技能)
        - [3.4.1.2. 被动技能](#3412-被动技能)
        - [3.4.1.3. 被动技能的类型](#3413-被动技能的类型)
        - [3.4.1.4. 如何实现被动技能](#3414-如何实现被动技能)
        - [3.4.1.5. 如何实现持续施法效果](#3415-如何实现持续施法效果)
        - [3.4.1.6. 持续施法效果和技能组的关系](#3416-持续施法效果和技能组的关系)
    - [3.4.2. 技能类的子类](#342-技能类的子类)
    - [3.4.3. 光环技能](#343-光环技能)
    - [3.4.4. 技能与状态的关系](#344-技能与状态的关系)
    - [3.4.5. 技能组设定](#345-技能组设定)
        - [3.4.5.1. 概述](#3451-概述)
        - [3.4.5.2. 技能组种类](#3452-技能组种类)
    - [3.4.6. 关于技能的延迟执行](#346-关于技能的延迟执行)
        - [3.4.6.1. 技能释放先后顺序](#3461-技能释放先后顺序)
        - [3.4.6.2. 技能的延迟执行](#3462-技能的延迟执行)
- [3.5. 伤害系统的设计](#35-伤害系统的设计)
- [3.6. 物品系统设计](#36-物品系统设计)
    - [3.6.1. 功能描述](#361-功能描述)
    - [3.6.2. 物品类（Item）属性](#362-物品类item属性)
    - [3.6.3. 物品格子类（ItemGrid）](#363-物品格子类itemgrid)
- [3.7. 商店系统设计](#37-商店系统设计)
    - [3.7.1. 功能描述](#371-功能描述)
    - [3.7.2. 优化方案](#372-优化方案)
    - [3.7.3. 具体实现](#373-具体实现)
        - [3.7.3.1. 商品复原功能](#3731-商品复原功能)
- [3.8. 装备合成系统设计](#38-装备合成系统设计)
    - [3.8.1. 为什么要有后缀装备这个属性](#381-为什么要有后缀装备这个属性)
- [3.9. 状态系统设计](#39-状态系统设计)
    - [3.9.1. 功能描述](#391-功能描述)
    - [3.9.2. 实现思路](#392-实现思路)
    - [3.9.3. 状态（BattleState）属性](#393-状态battlestate属性)
    - [3.9.4. 状态（BattleState）重要方法](#394-状态battlestate重要方法)
- [3.10. 单位](#310-单位)
    - [3.10.1. 普通单位Character](#3101-普通单位character)
    - [3.10.2. 英雄单位 Hero < Character](#3102-英雄单位-hero--character)
    - [3.10.3. 英雄单位相比普通单位有什么区别](#3103-英雄单位相比普通单位有什么区别)
    - [3.10.4. 英雄单位的特殊属性对基本属性的作用](#3104-英雄单位的特殊属性对基本属性的作用)
    - [3.10.5. 投射物Projectile](#3105-投射物projectile)
        - [3.10.5.1. 投射物的伤害计算](#31051-投射物的伤害计算)
        - [3.10.5.2. 投射物属性](#31052-投射物属性)
        - [3.10.5.3. 投射物的飞行轨迹](#31053-投射物的飞行轨迹)
        - [3.10.5.4. 投射物飞行轨迹实现](#31054-投射物飞行轨迹实现)
- [3.11. 玩家](#311-玩家)
    - [3.11.1. 描述](#3111-描述)
- [3.12. 音频管理与特效管理](#312-音频管理与特效管理)
    - [3.12.1. 音频管理](#3121-音频管理)
        - [3.12.1.1. 场景音频](#31211-场景音频)
        - [3.12.1.2. 单位音频](#31212-单位音频)
        - [3.12.1.3. UI音频](#31213-ui音频)
    - [3.12.2. 特效管理](#3122-特效管理)
        - [3.12.2.1. 如何管理特效对象的生命周期](#31221-如何管理特效对象的生命周期)
- [3.13. 战斗系统规则](#313-战斗系统规则)
    - [3.13.1. 判定伤害](#3131-判定伤害)
    - [3.13.2. 技能](#3132-技能)
- [4. 编辑器设计](#4-编辑器设计)
- [4.1. 编辑器技术选型](#41-编辑器技术选型)
- [4.2. 技能编辑器](#42-技能编辑器)
- [5. AI设计](#5-ai设计)
    - [5.0.1. ● NPC部分](#501-●-npc部分)
    - [5.0.2. 守卫型NPC行动分析（如：防守塔，野区野怪）](#502-守卫型npc行动分析如防守塔野区野怪)
    - [5.0.3. 进攻型普通AI行动分析（如：固定出兵攻打敌方基地的小兵）](#503-进攻型普通ai行动分析如固定出兵攻打敌方基地的小兵)
    - [5.0.4. 普通AI选择敌人策略](#504-普通ai选择敌人策略)
    - [5.0.5. 英雄AI设计](#505-英雄ai设计)
        - [5.0.5.1. AI所能获得的知识](#5051-ai所能获得的知识)
        - [5.0.5.2. AI所能做的操作](#5052-ai所能做的操作)
        - [5.0.5.3. AI行为树参考](#5053-ai行为树参考)
    - [5.0.6. 处理人物死亡逻辑](#506-处理人物死亡逻辑)
    - [5.0.7. ● 玩家部分](#507-●-玩家部分)
    - [5.0.8. 人物行动分析](#508-人物行动分析)
    - [5.0.9. 尝试使用行为树](#509-尝试使用行为树)
    - [5.0.10. 尝试使用状态机](#5010-尝试使用状态机)
        - [5.0.10.1. 状态机注意事项](#50101-状态机注意事项)
        - [5.0.10.2. 施法状态的编写](#50102-施法状态的编写)
        - [5.0.10.3. 施法状态的转移](#50103-施法状态的转移)
        - [5.0.10.4. 状态机图片](#50104-状态机图片)
- [6. 战争迷雾设计](#6-战争迷雾设计)
- [6.1. 原理](#61-原理)
- [6.2. 问题](#62-问题)
- [6.3. 解决方案](#63-解决方案)
...
- [7.2. 实现思路](#72-实现思路)
- [8. UI脚本设计](#8-ui脚本设计)
- [8.1. BattleStatusView（状态视图）](#81-battlestatusview状态视图)
    - [8.1.1. 功能描述](#811-功能描述)
    - [8.1.2. 实现](#812-实现)
- [8.2. 物品视图](#82-物品视图)
    - [8.2.1. 可执行的操作](#821-可执行的操作)
    - [8.2.2. 实现](#822-实现)
- [9. UI设计](#9-ui设计)
- [9.1. 自适应问题](#91-自适应问题)
    - [9.1.1. 问题提出](#911-问题提出)
    - [9.1.2. 问题解决](#912-问题解决)
    - [9.1.3. Vertical Layout Group 与 Horizontal Layout Group自适应的原理](#913-vertical-layout-group-与-horizontal-layout-group自适应的原理)
- [9.2. UGUI控件扩写](#92-ugui控件扩写)
    - [9.2.1. 指定输入类型InputField控件](#921-指定输入类型inputfield控件)
        - [9.2.1.1. 功能要求](#9211-功能要求)
    - [9.2.2. 显示图片选择框](#922-显示图片选择框)
        - [9.2.2.1. 功能要求](#9221-功能要求)
# 1. 游戏整体逻辑 #
## 1.1. 出兵逻辑 ##
...
### 3.1.1. 为什么要引入看起来变扭的Model类和Mono类 ###
...
: void 执行该技能效果
### 3.3.2. ***主动技能类 ActiveSkill < BaseSkill*** ###
1. 属性
	 1. 要消耗的魔法值 ： mp:int  :  <==
...
### 5.0.3. 进攻型普通AI行动分析（如：固定出兵攻打敌方基地的小兵） ###
...
### 9.1.3. Vertical Layout Group 与 Horizontal Layout Group自适应的原理
LayoutGroup多用来对复杂的UI进行布局（比如人物状态视图，或者类魔兽 Or RM的数据编辑器）


## 9.2. UGUI控件扩写
### 9.2.1. 指定输入类型InputField控件
#### 9.2.1.1. 功能要求
