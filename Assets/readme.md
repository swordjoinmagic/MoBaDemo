...
#### 基于享元模式编写的技能对象样例(Sample)
使用享元模式设计技能系统最重要的一点就是,可以很轻松的将技能(Skill)拷贝给某一对象(单位,技能组,物品,状态等),而无需使用深拷贝.
...
这样,就无需对Skill类中所有值类型进行深拷贝,优化了内存了.
...
## 编辑器与输入输出框架
数据编辑器与输入输出框架是紧密相连的，在本游戏Demo中，我简单的使用JSON作为游戏对象的数据来源，这样的话，就需要将编辑器中编辑好的数据转化为JSON文件（同时还需要将该JSON文件打包成AssetBundle文件，以供数据资源的动态更新）。

同时，打开编辑器后，还需要读取上一次已经编辑好的数据，来继续这一次的编辑。

这就需要有一套将编辑器数据输出为JSON的输出框架，以及将JSON文件输入进游戏创建游戏对象的输入框架。

### 将编辑器数据输出为JSON文件
目前游戏中大部分数据基本都是各个游戏对象字段的赋值，比如各个技能的各种属性赋值（技能名，消耗mp等）。

这就**需要知道对应对象中需要赋值的字段**是哪些，然后再在编辑器中以控件的形式表现出来，让数据编辑人员进行填写并保存。

这里我用**反射**的方法来获得这些需要填写的字段。

以技能对象举例：

1. 将技能对象中需要填写的属性均编写为C#的Attribute形式，这样就能直接反射拿到了。
2. 对于技能对象来说，他的Model类中有一个特殊的属性extraAttribute : Dictionary。这个属性主要用于保存技能子类（也就是各种操作）的各种接口属性。但是对于输出框架来说，这个不会构成问题，因为对于框架来说，他只需要知道目前要填什么字段就可以了。这个对于输入框架倒是一个问题。
3. 当所有字段填写完成后，数据编辑器自动将这些字段转化为json文件。

### 输入JSON文件并转化为游戏对象

同样以技能对象举例：

1. 当输入一个JSON文件后，首先根据该技能对象的skillType属性，反射创建对应的技能对象。
2. 然后根据对应的JSON键值生成对应的SkilModel对象，但是JSON文件中会包含特殊属性extraAttribute里的所有额外属性,即闪电链次数等。这里依旧用反射判断，如果可以在SkillModel对象中找到该属性，那么就不是特殊属性，反之则是特殊属性，直接对新建的SkillModel对象的map["attribute"] = xxx就OK了。
...
根据草图,可以发现编辑器的UI还是比较简洁.主要包含两大部分,一个是选项卡,一个是用于展示/编辑数据的面板.这里我简单的用一个UIManager进行管理就OK了.
